...
export async function GET(request) {
  ...
  const alcance = Math.min(Math.max(Number(parametros.get("range") || "3"), 3), 7);
  ...
  const datas = Array.from({ length: alcance * 2 + 1 }, (_, indice) => deslocarData(outboundDate, indice - alcance)).filter((data) => data >= hoje);
  try {
